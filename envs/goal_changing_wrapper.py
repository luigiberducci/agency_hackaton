...
class GoalChangingWrapper(gymnasium.Wrapper):
    def __init__(self, env: Env, goal_generator: str = None,p_change: float = 0.1, **kwargs):
    ...
    def step(self, action: Any) -> tuple[Any, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        if np.random.random() < self.p_change:
            self.env.goals = []
            self.env.grid.grid = [obj if not isinstance(obj, Goal) else None for obj in self.env.grid.grid ]
